...
<div class="auth-wrapper">
    ...
    <div class="auth-card">
        <div class="auth-header">
            ...
            <p class="auth-subtitle">Nhập Email mà bạn đã dùng để đăng ký tài khoản.</p>
        </div>
        ...
        <?php elseif (isset($multiple_accounts)): ?>
            <form action="<?php echo BASE_URL; ?>/account/forgotPassword" method="post">
                <input type="hidden" name="email" value="<?php echo htmlspecialchars($_POST['email']); ?>">
                <p style="color: var(--text); margin-bottom: 1rem; font-size: 0.95rem; text-align: center;">Vui lòng chọn tài khoản bạn muốn khôi phục:</p>
                <div style="margin-bottom: 1.5rem; display: flex; flex-direction: column; gap: 0.5rem;">
                <?php foreach($multiple_accounts as $acc): ?>
                    <label style="background: rgba(13,17,23,0.7); padding: 1rem; border-radius: 8px; border: 1px solid var(--border); display: flex; align-items: center; gap: 10px; cursor: pointer; transition: all 0.2s;">
                        <input type="radio" name="username" value="<?php echo htmlspecialchars($acc->username); ?>" required style="accent-color: var(--accent); transform: scale(1.2);">
                        <span style="color: #fff; font-weight: 600; font-size: 1.05rem;"><?php echo htmlspecialchars($acc->username); ?></span>
                    </label>
                <?php endforeach; ?>
                </div>
        ...
        <?php else: ?>
            <form action="<?php echo BASE_URL; ?>/account/forgotPassword" method="post">
                ...
                <div class="form-floating">
                    <input type="email" name="email" class="form-control-custom" placeholder="Nhập Email của bạn" required />
                </div>

                <button type="submit" class="auth-btn" style="margin-top: 1rem;">Gửi Link Khôi Phục</button>
                
                <div style="text-align: center; margin-top: 1.5rem; font-size: 0.85rem;">
                    <a href="<?php echo BASE_URL; ?>/account/login" style="color: var(--muted); text-decoration: none;">&larr; Trở về Đăng nhập</a>
                </div>
            </form>
        <?php endif; ?>
    </div>
